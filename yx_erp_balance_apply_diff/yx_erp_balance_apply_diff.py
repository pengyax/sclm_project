...
def load_erp_balance_apply_data(diff_days_start, diff_days_end, erp_con):
	...
	ISNULL( a.auditime, a.createTime ) audi_time,
	CONVERT(varchar(100), ISNULL( a.auditime, a.createTime ), 23) audi_date,
	IIF (
...
WHERE
	a.status = 1 
	AND Deleted = 0 
	AND datediff( dd, ISNULL( a.auditime, a.createTime ), getdate( ) ) <= { diff_days_start } 
	AND datediff( dd, ISNULL( a.auditime, a.createTime ), getdate( ) ) >= { diff_days_end }
	AND d.ActivityCWTypeNo in (1,2)
	AND agentname NOT LIKE '%%测试%%' 
	-- and c.UnionNumber not in ('CM00006317','CM00007650')
	AND Item in ('货物预存款','全品牌转款','销售订单撤销','代理清算','三合一转款','订单补差额','保证金')
...
def contrast_balance_apply(diff_days_start, diff_days_end):
    ...
    data = pd.merge(yx_balance_apply_data[yx_balance_apply_data.SourceNo.notnull()], erp_balance_apply_data[erp_balance_apply_data.SourceNo.notnull()], how='outer', left_on=
                    ['SourceNo','trans_date','UnionNumber','policy_name'],right_on=['SourceNo','audi_date','UnionNumber','zcname'], suffixes=('_yx', '_erp'))
    data['err_type'] = None
    ...
    todo = set(data['date'].astype('str').to_list())
    if len(todo) > 0:
        todo = "('"+"','".join(todo)+"')"
        bi_con.execute(
