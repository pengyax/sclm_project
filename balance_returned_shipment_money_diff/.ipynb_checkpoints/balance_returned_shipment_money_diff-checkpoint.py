...
def load_balance_data(date,diff,con):
	...
	-- 	and isnull(t1.CreateTime,CONVERT ( VARCHAR ( 10 ), DATEADD( DAY, - 1, t2.CreateTime ), 23 )) = '2021-04-13'
		and isnull(t2.balance,0)- isnull(t1.balance,0) != 0) a 
...
FROM
	(
	SELECT 
	CONVERT ( VARCHAR ( 10 ), isnull(c.FirstPayTime,c.createdTime ), 23 ) sendtime,
	a.agentWechat,
	a.agentId,
	a.agentname,
	e.zcname,
	...
	FROM
		...
		LEFT JOIN dbo.DH_Order_Detail d ON c.orderNo= d.orderNo 
	...
	WHERE
		...
		AND len( c.orderNo ) > 0 
		AND e.zctype = '混拿回款' 
		AND CONVERT ( VARCHAR ( 10 ), isnull(c.FirstPayTime,c.createdTime ) , 23 ) BETWEEN '{start_date}' and '{end_date}'
	GROUP BY
		CONVERT ( VARCHAR ( 10 ), isnull(c.FirstPayTime,c.createdTime ), 23 ),	a.agentWechat,a.agentname,e.zcname,a.agentId
	...
	FROM
		(
		SELECT CONVERT
			( VARCHAR ( 10 ), auditeTimeStorage, 120 ) sendtime,
			AfterNo,
			drawbackvalue 
		FROM
		...
		WHERE
			refuntype = '退货' 
			AND CONVERT ( VARCHAR ( 10 ), auditeTimeStorage, 120 ) BETWEEN '{start_date}' and '{end_date}'
		) a
		LEFT JOIN ( SELECT agentId, agentName,agentWechat,AfterNo, activityid FROM dbo.DH_AfterSale WHERE refundFlag = 88 ) b ON a.AfterNo = b.AfterNo
		LEFT JOIN bidata.bidata.dbo.ZCLX c ON b.activityid = c.zc_id 
	WHERE
		zctype = '混拿回款' 
	GROUP BY
		sendtime,agentWechat,agentName,c.zcname,agentId
	) a 
	LEFT JOIN DH_UserInfo b on a.agentid = b.id
GROUP BY
...
FROM
	...
	SUM ( ( d.totalAmount- isnull( d.apportionAmount, 0 ) ) / totalCount * b.count ) / 10000 AS money 
	FROM
		JKERPDB_UE.dbo.DH_Order_Shipment a
		LEFT JOIN dbo.DH_Order_Shipments_Relation b ON a.shipNo= b.shipNo
		LEFT JOIN dbo.DH_Order c ON a.orderNo= c.orderNo 
		AND b.orderNo= c.orderNo
		LEFT JOIN dbo.DH_Order_Detail d ON c.orderNo= d.orderNo 
	...
	FROM
		...
		SELECT CONVERT
			...
			drawbackvalue 
		FROM
			JKERPDB_UE.dbo.DH_AfterSale_Details 
		WHERE
			refuntype = '退货' 
			AND CONVERT ( VARCHAR ( 10 ), auditeTimeStorage, 120 ) BETWEEN '{start_date}' and '{end_date}'
		) a
...
def contrast(date,diff,con):
    ...
    if len(data)>0:
    	data.to_sql("InterMediate_balance_returned_shipment_money_diff", bi_con, index=False, if_exists='append', method='multi', chunksize=100)
    return data

def main():
    con = connect('new_ERP')
    date = datetime.datetime.today().date()
    contrast(date,30,con)
